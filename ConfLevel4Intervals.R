...
lower.wilson<-p.tilde - qnorm(p = 1-alpha/2) * sqrt(n) / 
  (n+qnorm(1-alpha/2)^2) * sqrt(pi.hat*(1-pi.hat) + 
                                  qnorm(1-alpha/2)^2/(4*n))
upper.wilson<-p.tilde + qnorm(p = 1-alpha/2) * sqrt(n) / 
  (n+qnorm(1-alpha/2)^2) * sqrt(pi.hat*(1-pi.hat) + 
                                  qnorm(1-alpha/2)^2/(4*n))

# Clopper-Pearson - This is a little more complicated due to the y = 0 
# and n cases
...
for(pi in pi.seq) {
  ...
  # Agresti-Coull
  save.AC<-ifelse(test = pi>lower.AC, yes = ifelse(test = pi<upper.AC, 
                                                   yes = 1, no = 0), no = 0)
  AC<-sum(save.AC*pmf)
  ...
  # Clopper-Pearson
  save.CP<-ifelse(test = pi>lower.CP, yes = ifelse(test = pi<upper.CP, 
                                                   yes = 1, no = 0), no = 0)
  CP<-sum(save.CP*pmf)
...
# pdf(file = "c:\\figures\\Figure1.3.pdf", width = 7, height = 6, 
...
# While AC and Wilson have same true confidence levels at pi=0.157, 
...
# Alternatively, these true confidence levels can be found 
# using the binom.coverage()
...
# from the binom.plot() function in the binom package. 
# Note that the method argument uses a function name
# that calculates the individual confidence interval. 
# Also, the y-axis is the true
# confidence level and the x-axis is pi. 
# The lattice package is used for plotting
# so the usual arguments (like xlab = ) for basic R plots 
# do not always work.  #
