...
# Wald statistic
Z.0<-(pi.hat1 - pi.hat2)/sqrt( pi.hat1*(1-pi.hat1)/sum(c.table[1,]) + 
                                 pi.hat2*(1-pi.hat2)/sum(c.table[2,]) )
Z.0^2

# Incorporate null hypothesis into variance in the denominator
pi.hat.Ho<-sum(c.table[,1])/sum(c.table)
Z.0<-(pi.hat1 - pi.hat2)/sqrt( pi.hat.Ho*(1-pi.hat.Ho)*(1/sum(c.table[1,])
                                                        + 1/sum(c.table[2,])) )
Z.0^2
...
#  for variance but not for the odds ratio itself unless there are 
#  0 counts (see code in function).
...
OR.tilde<-(c.table[1,1]+0.5)*(c.table[2,2]+0.5)/((c.table[1,2]+0.5)*
                                                   (c.table[2,1]+0.5))
log(OR.tilde)  # Does not match vcd's log(OR^)
sqrt(1/(c.table[1,1]+0.5) + 1/(c.table[2,2]+0.5) + 1/
       (c.table[1,2]+0.5) + 1/(c.table[2,1]+0.5))  # Matches vcd's 
# standard error (sqrt(var^(log(OR^)))
