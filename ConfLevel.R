...
save<-ifelse(test = pi>lower, yes = ifelse(test = pi<upper, yes = 1, no = 0), 
             no = 0)
save[1:10]
mean(save)
true.conf<-mean(save)
...
binom.confint(x = sum(save), n = numb.bin.samples, conf.level = 1-alpha, 
              methods = "wilson")
...
sum(prop.w)
sum(binom.prob)  # Note: not equal to 1 because some possible values of w 
# were not observed
