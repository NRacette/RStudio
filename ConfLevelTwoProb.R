...
n1<-25
n2<-25
numb.bin.samples<-2000  # Number of binomial samples
...
numb.bin.samples<-10000  # Number of binomial samples - 
# changed to reduce simulation variability (makes plot look nicer)
...
plot(x = save.true.conf[,1], y = save.true.conf[,2], xlab = expression(pi[1]),
     ylab = "Estimated true confidence level", type = "l", ylim = c(0.85,1), 
     lty = "solid", col = "blue")
lines(x = save.true.conf[,1], y = save.true.conf[,3], lty = "dashed", 
      col = "red")
...
lower<-pi.hat.all[,1] - pi.hat.all[,2] - qnorm(p = 1-alpha/2) * 
  sqrt(pi.hat.all[,1]*(1-pi.hat.all[,1]) / 
         n1 + pi.hat.all[,2]*(1-pi.hat.all[,2]) / n2)
upper<-pi.hat.all[,1] - pi.hat.all[,2] + qnorm(p = 1-alpha/2) * 
  sqrt(pi.hat.all[,1]*(1-pi.hat.all[,1]) / n1 + 
...
# Plot
x11(width = 7, height = 6, pointsize = 12)
# pdf(file = "c:\\figures\\Figure1.4color.pdf", width = 7, height = 6, 
# colormodel = "cmyk")   # Create plot for book
plot(x = save.true.conf[,1], y = save.true.conf[,2], xlab = expression(pi[1]),
...
# pdf(file = "c:\\figures\\Figure1.4BW.pdf", width = 7, height = 6, 
# colormodel = "cmyk")   # Create plot for book
plot(x = save.true.conf[,1], y = save.true.conf[,2], xlab = expression(pi[1]),
     ylab = "True confidence level", type = "l", ylim = c(0.85,1), 
     lty = "solid", col = "black")
...
pi1seq<-seq(from = 0.001, to = 0.999, by = 0.0025)  # using a smaller 
# "by" argument value can lead to slow rendering when rotating plots
# pi1seq<-seq(from = 0.1, to = 0.9, by = 0.1)  # Testing
...
# Save true confidence levels in a matrix
save.true.conf<-matrix(data = NA, nrow = length(pi1seq)*length(pi2seq), 
                       ncol = 4)
...
# write.table(x = save.true.conf, file = "c:\\chris\\save.true.conf.txt", 
# quote = FALSE, row.names = FALSE)
# save.true.conf<-read.table(file = "c:\\chris\\save.true.conf.txt", 
# header = TRUE)
...
persp3d(x = pi1seq, y = pi2seq, z = save.true.conf[,3], xlim = c(0,1), ylim =
          c(0,1), zlim = c(0.85, 1), xlab = "pi1", ylab = "pi2", 
...
# The zlim option in persp3d does not fix the axis limits like it should. Below
# is a fix
...
save.true.conf.wald2<-test.true.conf.wald*save.true.conf[,3]  # Put NA's 
# in vector if true confidence level is < 0.85
open3d()
...
# Note that there is not a par(mfrow = c(1,2)) type of option yet in the 
# rgl package, so I used a
# graphics editor to get the two plots side-by-side. Also, 
# I used a print screen to obtain
